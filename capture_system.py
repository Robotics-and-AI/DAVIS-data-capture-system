...
class CaptureSystem:
    ...
    def capture(self,recording_mode: int) -> None:
        ...
        try:
            self._start_paralel_thread(paralel_thread)
        except OSError:
            raise
    ...
    def recording_function(self, current_attempt, task_name, primitive, paralel_thread):
        ...
        self._record_with_jaer(paralel_thread)
        ...
        try:
            ...
            self._close_capture_system(paralel_thread)
    ...
    def _record_with_jaer(self, paralel_thread) -> None:
        ...
        try: 
            self._wait_for_button_input("pedal","Press the Pedal to start recording...", 500, 10, True, paralel_thread)
        except RuntimeError:
            self._close_capture_system(paralel_thread)
            raise
        ...
        try: 
            self._send_command_to_jaer((b"startlogging " + self._file_name),paralel_thread) 
        except OSError:
            self._close_capture_system(paralel_thread)
            raise     

        try:
            self._wait_for_button_input("pedal","Press the Pedal to stop recording...", 3000, 10, False, paralel_thread)
        except RuntimeError:
            self._close_capture_system(paralel_thread)
            raise
        ...
        try:
            self._send_command_to_jaer(b"stoplogging", paralel_thread)
        except OSError:
            self._close_capture_system(paralel_thread)
            raise 
    ...
    def _start_paralel_thread(self, paralel_thread: threading.Thread) -> None:
    ...
    def _wait_for_button_input(self, button:str, text: str, n_cycles: int, max_wait: int, condition: bool, paralel_thread) -> None:   
    ...
    def _send_command_to_jaer(self,command: bytes, paralel_thread) -> None:
    ...
    def _close_capture_system(self, paralel_thread:threading.Thread) -> None:
